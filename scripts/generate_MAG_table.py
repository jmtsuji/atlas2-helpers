...
GTDBTK_TAXONOMY_FILEPATHS = ['genomes/taxonomy/gtdb/gtdbtk.bac120.summary.tsv',
                             'genomes/taxonomy/gtdb/gtdbtk.ar122.summary.tsv']
...
def parse_gtdbtk_taxonomy_entry(taxonomy_entry, resolve=True):
    ...
    if '' in taxonomy_split and resolve is True:
        ...
        if False in empty_taxa[first_empty_taxon:]:
            logger.error('There seems to be an empty entry in the middle of your taxonomy levels. Cannot resolve. '
                         'Exiting...')
            logger.error('Entry was: ' + ', '.join(taxonomy_entry))
            sys.exit(1)
...
def load_and_parse_gtdbtk_taxonomy_table(gtdbtk_classification_filepaths, resolve=True):
    ...
    # Pass additional argments to map via lambda function -
    #  see https://stackoverflow.com/a/31995624 (accessed Sept. 18, 2019)
...
def parse_cat_taxonomy_row(cat_taxonomy_row, resolve=True):
    # E.g., Bacteria: 1.00    Nitrospirae: 0.95    Nitrospira: 0.91    Nitrospirales: 0.91    Nitrospiraceae: 0.91
    #       not classified    not classified
    # Goal: Bacteria    Nitrospirae    Nitrospira    Nitrospirales    Nitrospiraceae
    #       Unresolved_Nitrospiraceae    Unresolved_Nitrospiraceae
    ...
    if ('not classified' in cat_taxonomy_row) or ('NA' in cat_taxonomy_row) and resolve is True:
        ...
        # Only look at entries 1-6 for the initial check; sometimes CAT will leave a species level classification in
        # at position 7 But CAT does some odd things with taxonomy sometimes, so there might be a difficult to parse
        # entry as well.
        if False in empty_taxa[first_empty_taxon:len(empty_taxa) - 1]:
            # TODO - is there a more elegant way to handle these entries?
            logger.warning(
                'There seems to be an empty entry in the middle of one of your taxonomy levels. Cannot resolve...')
...
def load_and_parse_cat_taxonomy_table(cat_taxonomy_table_filepath, resolve=True):
    ...
    # Example
    # bin	superkingdom	phylum	class	order	family	genus	species MAG001    Bacteria: 1.00
    # Nitrospirae: 0.95    Nitrospira: 0.91    Nitrospirales: 0.91    Nitrospiraceae: 0.91    Nitrospira: 0.88    not
    #       classified
    ...
    taxonomy_table = pd.DataFrame.from_records(parsed_rows, columns=['MAG_ID', 'Domain', 'Phylum', 'Class',
...
def load_checkm_completeness_table(checkm_completeness_table_filepath):
    ...
    checkm_completeness_table.rename(columns={'Bin Id': 'MAG_ID',
                                     ...
                                     inplace=True)
    checkm_completeness_table = checkm_completeness_table[
        ['MAG_ID', 'Completeness', 'Contamination', 'Strain_heterogeneity']]
...
def main(args):
    ...
    if atlas_dir is not False:
        ...
        if (gtdbtk_taxonomy_filepaths is False) and (cat_taxonomy_filepath is False):
            ...
            for filepath in GTDBTK_TAXONOMY_FILEPATHS:
                filepath_full = os.path.join(atlas_dir, filepath)
                gtdbtk_taxonomy_filepaths.append(filepath_full)
    ...
    if gtdbtk_taxonomy_filepaths is not False:
        logger.info('GTDB-Tk taxonomy filepaths: ' + ', '.join(list(gtdbtk_taxonomy_filepaths)))
    else:
        logger.info('GTDB-Tk taxonomy filepaths: ' + str(gtdbtk_taxonomy_filepaths))
